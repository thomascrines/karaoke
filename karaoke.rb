...
class Karaoke
  ...
  attr_reader(:karaoke_name, :room)

  def initialize(karaoke_name, room)
    @karaoke_name = karaoke_name
    @room = room
  end

  def summarise_event(karaoke_name, room)
    return "You sang in the #{room} at the #{karaoke_name}.
    Your guests were #{room.list_guests_names}.
    ...
    if @room.guests_can_pay_for_room == true
      ...
      You can stay at the #{karaoke_name} until you work it off."
    end
  end
